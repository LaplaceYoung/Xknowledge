...
const parserPath = 'x-knowledge-extension/src/utils/markdownExport.ts';

const newContent = `import { ParsedTweet } from './twitterParser';

function generateSingleTweetMarkdown(tweet: ParsedTweet): string {
  let md = '---\n';
  md += \`id: "\${tweet.id}"\n\`;
...
export function generateMarkdown(tweets: ParsedTweet[]): string {
  if (!tweets || tweets.length === 0) return '';

  // If it's a single tweet (e.g., Copy to Clipboard), include frontmatter
  if (tweets.length === 1) {
    return generateSingleTweetMarkdown(tweets[0]);
  }
...
fs.writeFileSync(parserPath, newContent);
console.log('Markdown export engine updated for Obsidian/Notion compatibility.');
